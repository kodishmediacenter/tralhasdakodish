...
def addyoutube():
    tube  = os.path.join(xbmcvfs.translatePath("special://home/addons/webinterface.kodish/tube.xml"))
    dialog = xbmcgui.Dialog()
    link = dialog.input('[COLOR yellow]Cole um Video,Canal ou Playlist[/COLOR]','V7xri1t5yX4', type=xbmcgui.INPUT_ALPHANUM)
    titulo = dialog.input('[COLOR yellow]Digite nome do Conteudo[/COLOR]','Youtube Favorito', type=xbmcgui.INPUT_ALPHANUM)
...
def addDir_youtube(title,url,icon,fanart):
...
def getItems(items,fanart,pesquisa=False):
    ...
    for item in items:
        ...
        try:
            putube = re.compile('<putube>(.*?)</putube>',re.MULTILINE|re.DOTALL).findall(item)
            if len(putube)>0:
                putube = putube[0]
            else:
                putube = ''
        ...
        try:
            ...
            elif utube.startswith('PL') and not '&order=' in utube:
                link_youtube = 'plugin://plugin.video.youtube/playlist/'+utube+'/'
                #addLink(name.encode('utf-8', 'ignore'), link_youtube,subtitle,thumbnail,fanArt,desc,genre,date,credits,year,director,writer,duration,premiered,studio,rate,originaltitle,country,rating,userrating,votes,aired)
                addDir_youtube(title=name.encode('utf-8', 'ignore'),url=link_youtube,icon=thumbnail,fanart=fanArt)

            elif putube.startswith('PL') and not '&order=' in putube:
                link_youtube = 'plugin://plugin.video.youtube/playlist/'+putube+'/'
                #addLink(name.encode('utf-8', 'ignore'), link_youtube,subtitle,thumbnail,fanArt,desc,genre,date,credits,year,director,writer,duration,premiered,studio,rate,originaltitle,country,rating,userrating,votes,aired)
                addDir_youtube(title=name.encode('utf-8', 'ignore'),url=link_youtube,icon=thumbnail,fanart=fanArt)

            elif utube.startswith('UC') and len(utube) > 12:
                link_youtube = 'plugin://plugin.video.youtube/channel/'+utube+'/'
                #addLink(name.encode('utf-8', 'ignore'), link_youtube,subtitle,thumbnail,fanArt,desc,genre,date,credits,year,director,writer,duration,premiered,studio,rate,originaltitle,country,rating,userrating,votes,aired)
                addDir_youtube(title=name.encode('utf-8', 'ignore'),url=link_youtube,icon=thumbnail,fanart=fanArt)
    ...
    try:
        ...
        if exists == True:
            ...
            if fcheck and fcheck.read() == '1' and check_addon == 'true':
                ...
                link = getRequest2('https://gist.githubusercontent.com/hpalohomora/22aaf182da97f653d73bb6e14ebf1335/raw/5ec069bcab3e3e0efd43cdad9fcd9d0e78f909d4/arquivos_matrix_n%25C3%25A3o_excluir.txt','').replace('\n','').replace('\r','')
...
def SKindex():
    #addDir("[B]League Of Legends Worlds [/B][COLOR cyan][B]||| Hidra [/B][/COLOR][COLOR pink][B]Tuxedo |||[/B][/COLOR]",'',22,"https://escorenews.com/media/tournament/e5573.png",'',title_descricao,'','','','','','','','','','','','','','','','')
    #addDir("[B]Summer Eletrohits [/B][COLOR cyan][B]||| Hidra [/B][/COLOR][COLOR pink][B]Tuxedo |||[/B][/COLOR]",'',24,"https://macmagazine.com.br/wp-content/uploads/2011/12/13-icone-app-summer-eletrohits.png",'',title_descricao,'','','','','','','','','','','','','','','','')
    getData(url_principal, '')
    xbmcplugin.endOfDirectory(addon_handle)
    
def SKindex2():
    tube  = "http://localhost:8080/tube.xml"
    getData(tube, '')
    xbmcplugin.endOfDirectory(addon_handle)
