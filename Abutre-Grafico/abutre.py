...
def main(link,photo):
    ...
    titulo2 = re.sub('Torrent(.*)',r'',titulo)
    titulo = titulo2
    sint = len(sin2)
    ...
    filem.write('<link>')
    filem.write(magnet7)
    filem.write('$nome=[B]Op 1 1080[/B]</link>\n')
    filem.write('<link>'+trailerx+'$nome=[B]Trailer[/B]</link>\n')
    #filem.write('<link>'+trailerx+'$nome=[B]Data[/B]</link>\n')
    filem.write("<thumbnail>"+photo2+"|User-Agent=Mozilla/5.0 (Windows NT 10.0; WOW64; rv:47.0) Gecko/20100101 Firefox/47.0</thumbnail>\n")
    filem.write("<fanart></fanart>\n")
    filem.write("<info>"+sinopse+"</info>\n")
    filem.write("</item>")
