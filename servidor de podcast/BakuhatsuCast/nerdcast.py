...
site = "https://anchor.fm/s/3fc11f6c/podcast/rss"
...
for t in range(0,t2s):
    meta = str(t3[t].encode("utf-8"))
    podcast = open("servers.m3u", 'a')
    podcast.write('#EXTINF:-1 tvg-id="" tvg-logo="",'+meta.replace('CRONOL\\xc3\\x93GICA','CRONOLOGICA')+'')
    podcast.write(t2[t])
    podcast.close()
